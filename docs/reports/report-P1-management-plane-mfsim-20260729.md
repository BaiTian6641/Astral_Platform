...
| `make test-sv` | **12 SV TB 全过**（含新增 `tb_emri_regfile`、`tb_emri_occ_loop`） |
...
- **帧解码器 RTL**：OCC 帧总线（列打包帧）↔ fabric_top（tile 配置点）的桥接；打通后 `tb_emri_occ_loop` 可延伸到真实 `fabric_top`（目前 fabric 配置由 `tb_hotswap` 直接 cfg 端口证明，二者尚未 stitch）。
