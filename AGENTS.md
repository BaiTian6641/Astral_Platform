...
- **Fabric core in progress (2026-07-24):** `E0-FAB1` (eLUT4+FF) DONE — `ethereal-fabric/rtl/clb/elut4.sv` (G1-clean, in lint glob) + golden model `elut4_model.py` validated by **1211 local pytest cases**; cocotb DUT-vs-model (1000 configs) is Docker-gated. **Convention:** `make test-model` runs pure-Python golden-model pytest locally (no simulator); cocotb tests are `test_<unit>.py` (Docker-gated), model tests are `test_<unit>_model.py` (local). Next: `E0-FAB2` (CLB-T cluster).
