...
- **Fabric core in progress (2026-07-24):** `E0-FAB1` (eLUT4+FF) + `E0-FAB2` (CLB-T: N=8 eLUT4 + flat full-input crossbar IIB) DONE — `elut4.sv`/`clb_t.sv` G1-clean (in lint glob; CLB-T uses a scoped UNOPTFLAT waiver for the feedback loop) + golden models validated by **1218 local pytest cases** (incl. 832-route connectivity exhaustion); cocotb DUT-vs-model is Docker-gated. v1 IIB = flat crossbar (frozen, ASSUMPTION; two-level Clos is v2). **Convention:** `make test-model` runs pure-Python golden-model pytest locally; cocotb tests are `test_<unit>.py` (Docker-gated), model tests are `test_<unit>_model.py` (local). `E0-FAB3` (switch_box disjoint-unidir W=12 + `fabric_top` 4×4 grid) DONE — fabric-model Kahn cycle-detection validates **no comb loop** in default config; full model suite **1876 pass** locally; SB topology + full routable CB are VPR-pending/architecture items. Next: `S02-P0#1` (frame-map) / `E0-FAB4` (OCC v0).
