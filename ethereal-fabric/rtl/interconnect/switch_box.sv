...
module switch_box #(
    ...
    input  logic [$clog2(4*W)-1:0] cfg_addr_i,   // AW=$clog2(4*W)=6 for W=12 ; addr=DIR*W+t
    input  logic [1:0]             cfg_data_i,   // 0=disconnect, 1/2/3=the 3 sources
    ...
    // ---- derived parameters ----
    localparam int AW              = $clog2(4*W);        // 6 for W=12
    localparam int NSEL            = 4*W;                // 48 (one 2-bit sel per (DIR,t))
    localparam int DW              = 2;                  // select width
    localparam logic [AW-1:0] SB_END = NSEL[AW-1:0];    // 48 (< 2^AW for non-pow2 W)
    ...
    // ---- per-(DIR,t) select config (packed: 4*W selects x 2 bits) ----
    logic [NSEL*DW-1:0] sel_r;

    // ---- config write (no reset; OCC configures before run) ----
    always_ff @(posedge clk_i) begin
        if (cfg_we_i && (cfg_addr_i < SB_END)) begin
