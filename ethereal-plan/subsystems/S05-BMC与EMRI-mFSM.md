...
> ⚠️ **计划修订（ADR-018 已批准 rev3，2026-07-30）：** BMC 将经 **XBUS→AXI4 桥**成为系统 AXI 总线上的
> **一等主控**（全系统可达，可经 AXI 直驱 OCC/EMRI/region），其私有 UART/SPI/I2C 保留为管理
> 控制台。**NEORV32 仍是 BMC（ADR-016 不变）**；新增一个**独立的自研应用处理器子系统（S15）**：
> **自研 RV64（RVA23 方向）核心集群 `eth_rv`（≤4 核）**，用户可配置（RV32/RV64、单/多核、实时嵌入），
> 挂在**标准 AXI4 插座**后可插拔（类比 Zynq ARM PS）；并配**自研 DMA 子系统**（`eth_dma_mc` +
> `eth_dma_2d`）与**封装式 DRAM 组件 `eth_dram_ctrl`**。详见 `docs/adr/ADR-018-axi-noc-riscv-cluster.md`。
