...
ifeq ($(IVERILOG),)
	...
	@echo "[test-sv] bmc-fw (build real C firmware image)"; $(MAKE) -C ethereal-runtime/bmc-fw >/dev/null 2>&1 && cp ethereal-runtime/bmc-fw/build/bmc_fw.hex generated/bmc/bmc_fw.hex && echo "  firmware ok"
	@echo "[test-sv] tb_bmc_fw"; $(IVERILOG) -g2012 -o /tmp/tb_bmcfw ethereal-shell/rtl/bmc/bmc_core.sv ethereal-shell/rtl/bmc/neorv32_verilog_wrapper.v ethereal-shell/rtl/axi/eth_wb2axi.sv ethereal-fabric/tests/bmc/tb_bmc_fw.sv 2>/dev/null && vvp /tmp/tb_bmcfw | grep -q "TEST PASSED" && echo "  PASS"
