...
- **E0-MAP3 NEXT increments:** (3) LEVEL-2 frame packing (bitgen DB → frames via frame_map; validate round-trip + c17 logic-frame correctness). (4) routing bitgen (SB/CB/inject from VPR `.route` OR custom router on real fabric topology — ADR-012 refinement TBD) + IO path (primary I/O entry/exit, RTL not built) + sim harness → c432 bit-true in sim fabric (E0-MAP3 acceptance).
