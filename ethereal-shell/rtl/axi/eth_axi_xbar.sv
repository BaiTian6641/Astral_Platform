...
`ifdef FORMAL
    ...
    always_ff @(posedge clk_i) begin
        if (past_valid && $past(rst_ni) && rst_ni) begin
            ...
            for (pi = 0; pi < N_MST; pi++) begin
                if ($past(s_bvalid[pi]) && !$past(s_bready[pi])) begin
                    assert(s_bvalid[pi]);
                    assert(s_bid[pi*AXI_IDW +: AXI_IDW] == $past(s_bid[pi*AXI_IDW +: AXI_IDW]));
                end
                if ($past(s_rvalid[pi]) && !$past(s_rready[pi])) begin
                    assert(s_rvalid[pi]);
                    assert(s_rid[pi*AXI_IDW +: AXI_IDW] == $past(s_rid[pi*AXI_IDW +: AXI_IDW]));
                    assert(s_rdata[pi*AXI_DW +: AXI_DW] == $past(s_rdata[pi*AXI_DW +: AXI_DW]));
                end
            end
