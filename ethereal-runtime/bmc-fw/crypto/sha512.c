...
/*
 * sha512.c — freestanding SHA-512 (FIPS 180-4), portable C99, no libc, no
 * dynamic memory (G1). Straight-from-spec implementation: correctness first.
 *
...
static uint64_t rotr64(uint64_t x, uint32_t n) { return (x >> n) | (x << (64u - n)); }
...
static void sha512_compress(eth_sha512_ctx *ctx, const uint8_t *block)
{
    uint64_t w[80];
    for (int i = 0; i < 16; i++) {
        w[i] = load_be64(block + (size_t)i * 8u);
    }
    for (int i = 16; i < 80; i++) {
...
{
    /* bit-length counter (128-bit; len < 2^32 so carry out of lo fits) */
    ...
    while (len > 0) {
        uint32_t space = 128u - ctx->block_len;
...
void eth_sha512_final(eth_sha512_ctx *ctx, uint8_t out[64])
{
    ...
    uint32_t pad = (ctx->block_len < 112u) ? (112u - ctx->block_len)
                                           : (128u + 112u - ctx->block_len);
    uint8_t zeros[128]; /* pad length is 1..128 depending on block_len */
    zeros[0] = 0x80u;
    for (uint32_t i = 1; i < pad; i++) {
        zeros[i] = 0;
    }
    /* bit counter already tracked; update() would bump it again, so inline */
    {
        uint32_t n = pad;
        const uint8_t *p = zeros;
        while (n > 0) {
            uint32_t space = 128u - ctx->block_len;
    ...
    store_be64(ctx->block + 112, ctx->len_hi);
    store_be64(ctx->block + 120, ctx->len_lo);
    sha512_compress(ctx, ctx->block);

    for (int i = 0; i < 8; i++) {
        store_be64(out + (size_t)i * 8u, ctx->h[i]);
    }
}
...
void eth_sha512(const uint8_t *data, uint32_t len, uint8_t out[64])
{
    eth_sha512_ctx ctx;
    eth_sha512_init(&ctx);
    eth_sha512_update(&ctx, data, len);
